...
class MetStruct extends FFFirebaseStruct {
  MetStruct({
    bool? stressSet,
    bool? engagementSet,
    bool? interestSet,
...
void addMetStructData(
  ...
  if (met.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields = !forFieldValue && met.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final metData = getMetFirestoreData(met, forFieldValue);
  final nestedData = metData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields = met.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getMetFirestoreData(
  MetStruct? met, [
  bool forFieldValue = false,
]) {
  if (met == null) {
  ...
  final firestoreData = mapToFirestore(met.toMap());

  // Add any Firestore field values
  met.firestoreUtilData.fieldValues.forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getMetListFirestoreData(
  List<MetStruct>? mets,
) =>
    mets?.map((e) => getMetFirestoreData(e, true)).toList() ?? [];
